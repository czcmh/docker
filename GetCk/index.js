 ...
 * @return {*}
 */
async function cookieFlow(cookie) {
  const msg = await updateCookie(cookie);
  await sendMsg(msg, cookie);
  return msg;
}
    ...
    (async () => {
      try {
        ...
        if (cookie.body.errcode == 0) {
          ...
          const msg = await cookieFlow(ucookie);
          response.send({ err: 0, cookie: ucookie, msg });
        } else {
